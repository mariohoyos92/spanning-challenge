...
export default function ConsoleSide({ side, currentScreen }) {
  ...
  return (
    <div className="console">
      <button name={choices[side][currentScreen].one} />
      <button name={choices[side][currentScreen].two} />
      <button name={choices[side][currentScreen].three} />
      <button name={choices[side][currentScreen].four} />
    </div>
  );
}
